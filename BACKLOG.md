...
- [x] Support printing box labels to a 4x6 label printer (matching the validated label design: room-color band, Box ID, contents summary, QR code, "N of M" sequence, destination room, Fragile/Heavy/this-side-up badges, "if found contact" line, packed date). "This side up" is a new per-box flag alongside Fragile/Heavy; the "if found" line is a household-wide contact setting editable from the print screen. The QR code deep-links to the box's detail page. Printable from a single box (Box Detail) or in bulk for the currently filtered list (Box List) via `/print?ids=...`, one label per print page sized to 4in x 6in.
